...
class account_invoice_report(osv.osv):
	...
	def _select(self):
		select_str = """
			SELECT sub.id, sub.date, sub.product_id, sub.partner_id, sub.country_id, sub.supplier_invoice_number,
	...
	def _sub_select(self):
		select_str = """
				SELECT min(ail.id) AS id,
					...
					ail.product_id, ai.partner_id, ai.payment_term, ai.period_id, ai.supplier_invoice_number as supplier_invoice_number,
					u2.name AS uom_name,
	...
	def _group_by(self):
		group_by_str = """
				GROUP BY ail.product_id, ai.date_invoice, ai.id,
					ai.partner_id, ai.payment_term, ai.supplier_invoice_number, ai.period_id, u2.name, u2.id, ai.currency_id, ai.journal_id,
